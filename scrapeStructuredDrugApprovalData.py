...
def main():
	url = r'http://www.accessdata.fda.gov/scripts/cder/drugsatfda/index.cfm?fuseaction=Reports.NewOriginal_ANDA'
	domain = r'http://www.fda.gov/'

	page = urlopen(url).read()
	# data = urllib.urlencode({'MonthlyApprovalsAll':'3'})

	#Browse to the FDA Database
	browser = RoboBrowser(history=True)
	browser.open(url)

	drugTypeForm = browser.get_form(action="index.cfm?fuseaction=Reports.SetReport")
	drugTypeForm
	print(drugTypeForm)
	drugTypeForm['reportSelectMonth'].value = '7'
	drugTypeForm['reportSelectYear'].value = '2010'
	print(drugTypeForm)

	# drugTypeForm.serialize()
	browser.submit_form(drugTypeForm)
